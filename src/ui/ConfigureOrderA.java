...
import main.java.p1.IngredientsLasagna;
...
public class ConfigureOrderA extends JFrame implements ItemListener {
	...
	public ConfigureOrderA() {
		...
		JLabel lblPriceOrder = new JLabel("Price of the order: ");
		...
		btnNewButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Order o1 = new Order(Quantity, pizza);
				String texto = "";
	...
	public void itemStateChanged(final ItemEvent e) {
		...
		if (e.getSource() == comboBoxPizza) {
			...
			((AbstractPizza) pizza).removeIngrediente((IngredientType) comboBoxRemoveIngredients.getSelectedItem());
			for (Ingredients ing : ((AbstractPizza) pizza).getIngredientsList()) {
			...
			int q = Integer.parseInt(txtPizzaQuantity.getText());
			Quantity = q;
			Order order = new Order(q, pizza);

			txtPriceOrder.setText(order.getOrderCost() + "");

		} else if (e.getSource() == comboBoxAditionalIngredients) {

			((AbstractPizza) pizza).aditionIngrediente(
